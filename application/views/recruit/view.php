...
<html lang="ko">
	<head>
		...
		<style>
			thead { display: block; overflow: hidden; height: 80px; background: #d8b635; }
			tbody { display: block; overflow-y: scroll; height: calc(100vh - 100px); }
...
<?php foreach ($data as $r) { ?>
			...
			<tr class="row-2"><td><?=$r->email?></td><td><?=$r->message1?></td><td><?=$r->message2?></td><td><?=$r->message3?></td><td><?=$r->message4?></td></tr>
<?php } ?>
			</tbody>
		</table>
	</body>
